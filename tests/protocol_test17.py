...
Groups:
  ...
  C. version reporting -- startup ParameterStatus server_version is
     "0.20.0" (honest: not a fake "16.0"); SHOW server_version,
...
def group_c_version(c):
    ...
    check("startup server_version is 0.20.0",
          c.startup_params.get("server_version") == "0.20.0",
          str(c.startup_params.get("server_version")))
    msgs = c.query("SHOW server_version")
    check("SHOW server_version = 0.20.0", simple_single_value(msgs) == "0.20.0",
          str(simple_single_value(msgs)))
    msgs = c.query("SHOW server_version_num")
    check("SHOW server_version_num = 2000", simple_single_value(msgs) == "2000",
          str(simple_single_value(msgs)))
    msgs = c.query("SELECT version()")
    v = simple_single_value(msgs)
    check("version() mentions rustgres 0.20.0",
          v is not None and "rustgres 0.20.0" in v, str(v))

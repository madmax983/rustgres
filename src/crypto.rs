...
/// SHA-224 of `data` (FIPS 180-4): SHA-256 with distinct IV, truncated to
/// 28 bytes.
pub fn sha224(data: &[u8]) -> [u8; 28] {
    ...
    // Same padding and compression as SHA-256.
    let mut msg = Vec::with_capacity(((data.len() + 9 + 63) / 64) * 64);
    msg.extend_from_slice(data);
    msg.push(0x80);
    while msg.len() % 64 != 56 {
    ...
    let mut out = [0u8; 28];
    for (i, word) in h[..7].iter().enumerate() {
        out[4 * i..4 * i + 4].copy_from_slice(&word.to_be_bytes());
    }
    out
}
...
pub fn sha512(data: &[u8]) -> [u8; 64] {
    ...
    let mut msg = Vec::with_capacity(((data.len() + 17 + 127) / 128) * 128);
    msg.extend_from_slice(data);
    msg.push(0x80);
...
/// SHA-384 of `data` (FIPS 180-4): SHA-512 with distinct IV, truncated to
/// 48 bytes.
pub fn sha384(data: &[u8]) -> [u8; 48] {
    // Compute SHA-512 with SHA-384 IV, then truncate. We replicate the
    // compression here with the SHA-384 initial hash value.
    let mut h: [u64; 8] = [
        0xcbbb9d5dc1059ed8,
        0x629a292a367cd507,
        0x9159015a3070dd17,
